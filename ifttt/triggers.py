...
class TrendingTopics(BaseTriggerView):
    ...
    def get_data(self):
        ...
        return list(
            filter(self.only_trending, list(map(self.parse_result, resp["pages"])))
        )
    ...
    def only_trending(self, page):
        ...
        return (
            page["edits"] >= min_edits
            and page["editors"] >= min_editors
            and page["score"] >= min_score
            and title_match
        )
    ...
    def parse_result(self, page):
        ...
        return {
            ...
            "meta": {"id": url_to_uuid5(url), "timestamp": iso8601_to_epoch(updated)},
        }
...
class NewArticle(BaseAPIQueryTriggerView):
    ...
    def parse_result(self, rev):
        ret = {
            "date": rev["timestamp"],
            "url": "https://%s/wiki/%s" % (self.wiki, rev["title"].replace(" ", "_")),
            "user": rev["user"],
